...
/*
	forAll(T, cellI)
	{
		const labelList& neigh = mesh.cellCells();
		forAll(neigh, cellJ)
		{
			Tneighbor_sum[cellI] += T[ neigh[cellJ] ];
		}
	}
*/
/*
        const labelLabelList& neigh = mesh.cellCells();
	forAll(T, cellI)
	{		
		const labelList& cells = cellCells[cellI];;
		forAll(neigh, cellJ)
		{
			Tsum[cellI] += T[ neigh[cellJ] ];
		}
	}
*/
